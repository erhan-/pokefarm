...
def getLocation(search):
    loc = geolocator.geocode(search, timeout=10)
...
def filtered_forts(origin, forts):
    forts = [(fort, distance_in_meters(origin,(fort['latitude'], fort['longitude']))) for fort in forts if fort.get('type',None) == 1 and fort.get('enabled',None) and fort.get('cooldown_complete_timestamp_ms',-1) < time()*1000]
